...
for your testing and the other is the actual data you need to process for the assignment.
    ...
    Actual data: http://py4e-data.dr-chuck.net/regex_sum_1209912.txt 
    	(There are 73 values and the sum ends with 651)

These links open in a new window. Make sure to save the file into the same folder as you 
will be writing your Python program. Note: Each student will have a distinct data file 
...
random numbers are inserted throughout the text. Here is a sample of the output you might 
see:
...
for line in fh:
	# Put lst of nums in storage
    storage = re.findall('[0-9]+', line)
    # Use nested loop to count and find sum
    for nmbrs in storage:
        # Cnvrt Str nums to Int
        ans = ans + (int(nmbrs))
